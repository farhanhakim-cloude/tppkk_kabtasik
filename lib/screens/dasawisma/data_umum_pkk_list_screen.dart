...
class _DataUmumPkkListScreenState extends State<DataUmumPkkListScreen> {
  ...
  Future<void> _openForm({DataUmumPkkItem? item}) async {
    ...
    final res = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            DataUmumPkkFormScreen(level: _selectedLevel, item: item),
      ),
    );
    if (res == true) _reload();
  }
          ...
          Expanded(
            child: FutureBuilder<List<DataUmumPkkItem>>(
              ...
              builder: (context, snapshot) {
                ...
                final totalDasawisma = allList.fold(
                  0,
                  (p, e) => p + e.jumlahDasaWisma,
                );
                ...
                final totalSekretariat = allList.fold(
                  0,
                  (p, e) => p + e.totalSekretariat,
                );
  ...
  Widget _buildCardListView(List<DataUmumPkkItem> list) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final title = isDesa
            ? (item.namaDusun.isNotEmpty
                  ? item.namaDusun
                  : 'Dusun/Lingkungan ${index + 1}')
