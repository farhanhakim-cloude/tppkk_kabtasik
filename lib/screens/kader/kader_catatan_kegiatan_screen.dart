...
class _KaderCatatanKegiatanScreenState extends State<KaderCatatanKegiatanScreen>
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Container(padding: const EdgeInsets.all(9), decoration: BoxDecoration(color: const Color(0xFF0D9488).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.tune_rounded, size: 18, color: Color(0xFF0D9488))),
                const SizedBox(width: 10),
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Filter', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w800, fontSize: 16, color: const Color(0xFF0F172A))),
                  Text('Saring catatan berdasarkan kategori', style: GoogleFonts.plusJakartaSans(fontSize: 12, color: const Color(0xFF64748B))),
                ]),
              ]),
              const SizedBox(height: 16),
  ...
  Widget _buildFilterSheetItem(PokjaKategori? pokja, String title, String subtitle, IconData icon, Color color, BuildContext sheetCtx) {
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(width: 36, height: 36, decoration: BoxDecoration(color: isSel ? color.withValues(alpha: 0.15) : Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFFE2E8F0))), child: Icon(icon, size: 18, color: color)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700, fontSize: 13, color: const Color(0xFF0F172A))),
  ...
  Widget build(BuildContext context) {
    ...
    final searchBg = _isDarkMode ? const Color(0xFF252B36) : const Color(0xFFF1F5F9);
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          icon: Icon(Icons.arrow_back_ios_new_rounded,
              color: _isDarkMode ? Colors.white : const Color(0xFF0F172A), size: 20),
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintStyle: GoogleFonts.plusJakartaSans(fontSize: 13, color: subtextColor),
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      child: Stack(
                        ...
                        children: [
                          Icon(Icons.tune_rounded, size: 20, color: _selectedFilter == null ? subtextColor : Colors.white),
                          if (_selectedFilter != null)
                            Positioned(top: 10, right: 10, child: Container(width: 8, height: 8, decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: primaryAccent, width: 1.5)))),
                        ],
                      ),
                    ),
                  ),
              ...
              child: Row(children: [
                if (_selectedFilter != null)
                  Container(
                    ...
                    decoration: BoxDecoration(color: _getPokjaColor(_selectedFilter!).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(20)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(_getPokjaIcon(_selectedFilter!), size: 12, color: _getPokjaColor(_selectedFilter!)),
                      const SizedBox(width: 6),
                      Text(_selectedFilter!.shortLabel, style: GoogleFonts.plusJakartaSans(fontSize: 11.5, fontWeight: FontWeight.w700, color: _getPokjaColor(_selectedFilter!))),
                      const SizedBox(width: 6),
                      GestureDetector(
                        onTap: () => setState(() => _selectedFilter = null),
                        child: Icon(Icons.close_rounded, size: 14, color: _getPokjaColor(_selectedFilter!)),
                      ),
                    ]),
                  ),
                if (_selectedFilter != null && _searchQuery.isNotEmpty) const SizedBox(width: 8),
                if (_searchQuery.isNotEmpty)
                  Expanded(child: Text('Pencarian: "$_searchQuery"', maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 11.5, color: subtextColor))),
              ]),
            ),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.assignment_outlined, size: 52, color: accentColor),
            ),
            const SizedBox(height: 20),
            Text('Belum Ada Catatan Kegiatan',
                style: GoogleFonts.plusJakartaSans(
                    fontSize: 17, fontWeight: FontWeight.w700, color: textColor)),
            const SizedBox(height: 8),
            Text('Tekan tombol "Input Kegiatan" di bawah\nuntuk melaporkan kegiatan pokja.',
